...
unsigned int get_key(char *);
unsigned int recieve_request();
void perform_checks(unsigned int);
...
int main(int argc, char **argv) {
  ...
  printf("\n>----------\n");
  unsigned int secret = recieve_request();
  perform_checks(secret);
...
void perform_checks(unsigned int secret) {
  // perform checks
  ...
  unsigned int secret_key = get_key(ip);
  unsigned int ip_32 = inet_addr(ip);
  ip_32 = htonl(ip_32);
  secret_key = htonl(secret_key);
  int rec_ip = (ip_32 ^ secret_key);
  if (rec_ip != secret) {
    //close the connection
...
unsigned int get_key(char *ip) {
  // open acl.dat to get the key
  FILE *fp = fopen("acl.dat", "r");
  int fd = fileno(fp);
  int flag = 1;
  char key[10];
  while (flag == 1) {
    ...
    while (1) {
      ...
      if (feof(fp)) {
        ...
        // first time check if the ip are the same
        // if they are the same, read and store the key and berak
        // if not ignore everything else till \n
        temp_ip[i] = '\0';
        printf("IPs are %s %s\n", ip, temp_ip);
        if (strcmp(ip, temp_ip) == 0) {
          // read in and store the key and break;
          // read in the - and ' '
          temp = fgetc(fp);
          temp = fgetc(fp);
          int j = 0;
          ...
          // ignore everything and go to next line
          while ((temp = fgetc(fp)) != '\n') {
            // ignore everything
          }
        }
