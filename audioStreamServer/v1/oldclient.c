...
unsigned short getpspace() {
    ...
    if(method == 1) {
        // printf("Lambda: %f , s: = pspace: %f b: = %f\n", lambda, (epsilon*(targetbuf - currbuf)/blocksize_num), beta*(lambda - gamma));
        lambda = (lambda + (epsilon*(targetbuf - currbuf)/blocksize_num) - beta*(lambda - gamma));
    }
    sem_post(&mutex);
    if(lambda < 0 ) {
        lambda = 0.0;
...
void sendack(unsigned short value) {
    ...
    // unsigned int chosenPort = (unsigned int) ntohs(recvaddr.sin_port);
    // printf("port : %u\n", chosenPort);
...
//             &len);
//     // unsigned int chosenPort = (unsigned int) ntohs(recvaddr.sin_port);
//     // printf("Recieved port : %u\n", chosenPort);
//     if(n<0) {
//         // perror("Recevied error");
//     }
...
//                 filledbuf +=n;
//                 // logevent();
//                 // printf("Filled : %lu Filled till : %lu\n",filledbuf, currbuf);
//             }
//             sem_post(&mutex);
//         }
//         sendack(getpspace());
...
void handle_sigalarm(int sig) {
    ...
    bzero(buf, bufsize);
    printf("filledbuf=%lld\n",filledbuf);
    // printf("In alarm bufsize=%d sizeofbuf=%u\n", bufsize, sizeof(buf));
    if(filledbuf >= 4096) {
        // printf("In alarm2\n");
        //printf("Streamaudio filledbuf=%lld, file_read_done=%d\n",filledbuf, file_read_done);
        sem_wait(&mutex);
    ...
    free(buf);
//    mulawclose();
//    if(file_read_done && filledbuf < 4096) {
//            printf("Exit from stream\n");
//        exit(0);
//    }
}
...
void streamaudio() {
    ...
    while(!file_read_done) {
        ...
        // printf("blocksize_num = %d, filledbuf=%lld, sum=%lld\n",blocksize_num, filledbuf, filledbuf+blocksize_num);
        // if(filledbuf + blocksize_num <= windowsize_num ) {
            // printf("Hello");
            n = recvfrom(sockfd, (char *)buffer, blocksize_num, 
            0, (struct sockaddr *) &recvaddr,
            &len);
        ...
        if(n>0){
        // while(n > 0 && !packet_read) {
            // printf("n = %d, filledbuf=%lld, sum=%lld\n",n, filledbuf, filledbuf+n);
            sem_wait(&mutex);
            if(filledbuf + n <= windowsize_num) {
                currbuf = currbuf%windowsize_num;
                if((windowsize_num - currbuf) < n) {
